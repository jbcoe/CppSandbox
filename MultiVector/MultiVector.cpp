...
{
  using Data = decltype(
      std::tuple_cat(std::tuple<std::vector<T>>(),
...
struct BuildReferenceTuple
{
  ...
  static auto DoBuildReferenceTuple(std::tuple<Ts...>& tpl, size_t i)
  {
    return std::tuple_cat(
        std::forward_as_tuple(std::get<N>(tpl)[i]),
...
public:
  ...
  ~RestoreSize()
  {
    ...
    ApplyToAll<std::tuple_size<TupleT>::value>::DoApply(*data_,
                                                        [n = n_](auto& v)
                                                        {
                                                          v.resize(n);
                                                        });
...
struct MultiVector
{
  ...
  {
    return BuildReferenceTuple<0, sizeof...(Ts)>::DoBuildReferenceTuple(data_,
                                                                        i);
  }

  std::tuple<const Ts&...> operator[](size_t i) const
  {
    return BuildConstReferenceTuple<
        0, sizeof...(Ts)>::DoBuildConstReferenceTuple(data_, i);
  }
