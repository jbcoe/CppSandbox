...
class TObjectSetImpl : public SetInterface_t
{
		...
		class concrete_iterator : public IObjectSet::IteratorImpl
	{
		public:
			...
			concrete_iterator(std::vector<std::unique_ptr<CObjectImpl>>::iterator it) : m_it(it) {}
	...
	public:
		...
		{ 
			return IObjectSet::iterator(std::unique_ptr<concrete_iterator>(new concrete_iterator(m_vObjects.begin()))); 
		}
