...
class TupleUtils
{
	...
	template <typename Functor, typename Tuple_t, std::size_t... Indices>
		static auto apply_f_impl(Functor f, Tuple_t& t, TupleIndices<Indices...>) -> decltype(std::make_tuple(f(std::get<Indices>(t))...))
		{
	...
	template <typename Functor, typename ...Ts>
		static auto ApplyFunction(Functor f, const std::tuple<Ts...>& t) -> decltype(apply_f_impl(f, t, IndicesFor<Ts...> ()))
		{
