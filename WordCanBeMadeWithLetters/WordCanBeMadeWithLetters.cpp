...
int main(int argc, char* argv[])
{
  if ( argc == 1 )
	{
		cerr << "Needs a letter list as first argument and a word list piped from stdin" << endl;
		exit (1);
	}
  ...
  transform(letters.begin(), letters.end(), letters.begin(), [](char c){ return tolower(c);});
	...
	sort(results.begin(), results.end(), [](const string& s1, const string& s2) -> bool
	{
		if ( s1.size() < s2.size() )
			return true;
