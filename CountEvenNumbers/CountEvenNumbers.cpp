...
class ConditionalIterator
{
	public:
		...
		ConditionalIterator(Iterator_t iterator, Iterator_t end, Functor_t f) : iterator_(iterator), end_(end), f_(f) 
		{
			if ( ! f_(*iterator_) )
			{
				while ( iterator_!=end_ && ! f_(*iterator_) )
...
template <Iterator Iterator_t, Functor Functor_t> 
	std::pair<ConditionalIterator<Iterator_t,Functor_t>, ConditionalIterator<Iterator_t,Functor_t>> 
make_conditional_begin_and_end(Iterator_t begin, Iterator_t end, Functor_t condition)
{
	return { ConditionalIterator<Iterator_t,Functor_t>(begin,end,condition), ConditionalIterator<Iterator_t,Functor_t>(end,end,condition)};
}
...
int main(int argc, char* argv[])
{
  ...
  auto even_number_iterators = make_conditional_begin_and_end(numbers.begin(),numbers.end(),[](int x){return x%2==0;});

	int count = 0;
	auto even_number_with_count_iterators = make_transform_iterator_begin_and_end(even_number_iterators.first, even_number_iterators.second, 
			[&](int x) -> std::string
			{
				std::stringstream ss;
				ss << x << ":" << count++;
	...
	std::copy(even_number_with_count_iterators.first, even_number_with_count_iterators.second, osi);
	std::cout << std::endl;
}
