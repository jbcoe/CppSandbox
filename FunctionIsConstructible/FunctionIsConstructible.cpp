...
int main(int argc, char* argv[])
{
  ...
  std::cout << "std::is_constructible<std::function<int(int,int)>, "
               "decltype(f_1)>::value "
            << std::is_constructible<std::function<int(int, int)>,
                                     decltype(f_1)>::value << "\n";
  ...
  // std::function<int(int)> f_i_ii(f_2); // does not compile as it is not
  // constructible

  // std::function<int(int,int)> f_ii_i(f_1); // does not compile as it is not
  // constructible
