                              ...
                              std::stack<std::function<void()>>& fakeStack)
{
  ...
  if (node.right_)
  {
    fakeStack.push([&]
                   {
                     PrintNodes_FakeRecursive(*node.right_, fakeStack);
                   });
  }
  ...
  if (node.left_)
  {
    fakeStack.push([&]
                   {
                     PrintNodes_FakeRecursive(*node.left_, fakeStack);
                   });
  }
}
...
void AddNodes(Node& root, Generator_T&& generator, size_t levels)
{
  ...
  if (root.left_)
  {
    AddNodes(*root.left_, generator, levels - 1);
  }

  root.right_ = generator();
  if (root.right_)
  {
    AddNodes(*root.right_, generator, levels - 1);
  }
}
